function [BestSol Convergence_curve Positions_code]=BBES(nPop,MaxIt,low,high,dim,fobj,Bits)
% Hamming-based DBES
...
BestSol.cost = inf;
% Means = zeros(500,30);
Positions_code = inf(MaxIt,nPop,2);
for i=1:nPop
...
for t=1:MaxIt
    Positions_code(t,:,:) = [pop.pos(:,1),pop.pos(:,2)];
    %%               1- select_space 
    ...
    %%                3- swoop
  [pop BestSol s3(t) mean_ ]=swoop(fobj,pop,BestSol,nPop,low,high,Bits,dim,t);
...
function [pop best s1 mean_]=swoop(fobj,pop,best,npop,low,high,Bits,dim,t)
BMean = mean(pop.Bpos);
mean_=mean(pop.pos);
